...
public class CustomEventAdapter extends ArrayAdapter<BorrowList> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        TextView borDate = rowView.findViewById(R.id.tvBorDate);
        TextView retDate = rowView.findViewById(R.id.tvRetDate);

        borName.setText(values.get(position).borrower);
        bookName.setText(values.get(position).bookname);
        borDate.setText(values.get(position).borrowdate);
        retDate.setText(values.get(position).returndate);
